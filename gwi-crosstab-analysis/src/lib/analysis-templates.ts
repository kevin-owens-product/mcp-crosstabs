...
const AudienceProfilingTemplate: AnalysisTemplate = {
  ...
  applicableWhen: (crosstab: Crosstab) => {
    // Applies when analyzing a single audience across multiple attributes
    return crosstab.bases !== undefined && crosstab.bases.length === 1 &&
           crosstab.rows.length > 5;
  },

  analyze: (crosstab: Crosstab, _analysis: Analysis): TemplateAnalysis => {
...
const MarketComparisonTemplate: AnalysisTemplate = {
  ...
  analyze: (crosstab: Crosstab, _analysis: Analysis): TemplateAnalysis => {
    ...
    // Group by market
    const byMarket: { [key: string]: typeof data } = {};
    markets.forEach(m => {
      byMarket[m] = data.filter(d => d.segment === m);
    });

    // Find market-specific strengths
    const marketInsights: { [key: string]: {
      sampleSize: number;
      avgIndex: number;
      topBehaviors: typeof data;
    } } = {};
    ...
    const universalBehaviors: { datapoint: string; avgIndex: number }[] = [];
    ...
    const marketSpecific: { [key: string]: { datapoint: string; index: number }[] } = {};
    for (const dp of datapoints) {
      const marketIndexes = markets.map(m => ({
        market: m,
      ...
      if (max > 130 && marketIndexes.filter(mi => mi.index > 120).length === 1 && maxMarket) {
    ...
    const keyMetrics: KeyMetric[] = markets.map(market => ({
      ...
      significance: marketInsights[market].avgIndex > 110 ? 'positive' as const :
                   marketInsights[market].avgIndex < 90 ? 'negative' as const : 'neutral' as const
    }));
...
const TrendAnalysisTemplate: AnalysisTemplate = {
  ...
  analyze: (crosstab: Crosstab, _analysis: Analysis): TemplateAnalysis => {
    ...
    // Group by wave
    const byWave: { [key: string]: typeof data } = {};
    waves.forEach(w => {
...
const CompetitiveComparisonTemplate: AnalysisTemplate = {
  ...
  analyze: (crosstab: Crosstab, _analysis: Analysis): TemplateAnalysis => {
    ...
    const competitorProfiles: { [key: string]: {
      id: string;
      avgIndex: number;
      topStrengths: typeof data;
      sampleSize: number;
      dataPoints: number;
    } } = {};
    ...
    const competitiveAdvantages: { [key: string]: typeof data } = {};

    competitors.forEach(comp => {
      const profile = competitorProfiles[comp.name];
      const advantages: typeof data = [];
    ...
    return {
      ...
      keyMetrics: competitors.map(comp => {
        ...
        return {
          ...
          significance: profile.avgIndex > 110 ? 'positive' as const :
                       profile.avgIndex < 90 ? 'negative' as const : 'neutral' as const
        };
      }),
...
const MediaConsumptionTemplate: AnalysisTemplate = {
  ...
  analyze: (crosstab: Crosstab, _analysis: Analysis): TemplateAnalysis => {
    ...
    const totalMediaMix = totalSocial + totalDigital + totalTraditional;
    ...
    return {
      ...
      insights: [
        ...
        `**Media Mix Balance**: ${Math.round((totalSocial + totalDigital) / (totalMediaMix || 1) * 100)}% digital vs traditional`,
      ...
      recommendations: [
        ...
        `**Budget Allocation**: ${Math.round(totalSocial / (totalMediaMix || 1) * 100)}% social, ${Math.round(totalDigital / (totalMediaMix || 1) * 100)}% digital, ${Math.round(totalTraditional / (totalMediaMix || 1) * 100)}% traditional`,
...
export class TemplateAnalysisEngine {
   ...
   */
  selectTemplates(crosstab: Crosstab): AnalysisTemplate[] {
   ...
   */
  analyzeWithTemplates(crosstab: Crosstab, baseAnalysis: Analysis): {
  ...
  formatTemplateAnalysis(
    ...
    if (analysis.keyMetrics.length > 0) {
      ...
      analysis.keyMetrics.forEach(metric => {
        const indicator = metric.significance === 'positive' ? '[+]' :
                     metric.significance === 'negative' ? '[-]' : '[=]';
        output += `${indicator} **${metric.label}**: ${metric.value}\n`;
        if (metric.context) {
